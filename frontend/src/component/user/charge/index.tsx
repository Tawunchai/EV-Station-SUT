...
const ChargingEV = () => {
  ...
  const checkSession = useCallback(async () => {
    ...
    console.log("🔍 กำลังหาว่า session :", sessions);

    if (sessions.length > 1 && chargerId) {
    ...
    console.log("🔍 session after filter by chargerId:", targetSession);
  ...
  // ✅ NEW: เข้าหน้านี้แล้วดึง Snapshot เอา "แค่ energy_wh" มาเป็น currentEnergyWh
  // เพื่อให้ % สามารถคำนวณได้ทันทีแม้ยังไม่ทันได้ MeterValues ผ่าน WS
  useEffect(() => {
    const fetchSnapshotEnergy = async () => {
      try {
        if (!chargerId) return;
        if (freezeInterruptionRef.current) return;

        const res: any = await GetOcppSnapshotByChargerID(chargerId);
        const energyWh = Number(res?.snapshot?.meter?.energy_wh);

        if (Number.isFinite(energyWh)) {
          setCurrentEnergyWh(energyWh);
          console.log("⚡ [Snapshot] meter.energy_wh:", energyWh);
        }
      } catch (err) {
        console.error("❌ GetOcppSnapshotByChargerID error:", err);
      }
    };
